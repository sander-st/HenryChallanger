...
function palabraMasLarga(array) {
  ...
  const countWord = ar => ar.reduce((prev, current)=> Math.max(prev, current.length), 0)
  const filterWord = (ar, len) => ar.filter(val => val.length == len)[0]

  let bigWords = array.map(val => {
    let split = val.split(' ')

    return filterWord(split, countWord(split))
  })

  return filterWord(bigWords, countWord(bigWords))
